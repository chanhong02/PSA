...
def psi_value(Cp_air, Tatm, Patm, Pout, U_cp, gamma):
    P_ratio = Pout / Patm
    exponent = (gamma - 1) / gamma
    top = Cp_air * Tatm * (P_ratio**exponent - 1)
    bottom = 0.5 * U_cp**2
    return top / bottom

def mach_number(U_cp, Tatm):
    M = U_cp / np.sqrt(gamma * R_air * Tatm)
    return np.clip(M, 0.01, 1.5)
...
def phi_value(U_cp, T_cp_in, psi):
    M = mach_number(U_cp, T_cp_in)
    phi_m = phi_max(M)
    beta_m = beta_poly(M)
    psi_m = psi_max(M)
    ...
    exponent = beta_m * (psi / psi_m - 1)
    exponent = np.clip(exponent, -100, 50)  # overflow 방지
